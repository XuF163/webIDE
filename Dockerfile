...
# Install CC Switch (optional GUI AppImage; disabled by default for headless builds)
RUN if [ "${INSTALL_CC_SWITCH}" = "1" ]; then \
    set -eux; \
    ...
    echo "Skipping cc-switch install (set --build-arg INSTALL_CC_SWITCH=1 to enable)"; \
  fi
